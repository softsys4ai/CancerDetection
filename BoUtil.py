...
def fitCV(configs, cv=None, X_train=None, y_train=None, domain=None, clf_name="RF"):
...
def evaluation(
        ...
        saveFileNamePrefix, figID, testDataTags=None, oriDF=None):
    ...
    resultFDName = "result_"+labelName+"_"+sourceDataTag
    ...
    for nfIdx in range(numFGs):
        ...
        opt = GPyOpt.methods.BayesianOptimization(
                f = partial(fitCV, X_train=X_train, y_train=y_train, cv=cv, domain=domain, clf_name=clf_name),  # function to optimize
...
#        numFGs, numFeats, test_ratio, cv,
#        domain, clf_name, itersBO,
#        saveFileNamePrefix, figID, testDataTags=None):
#
#    if testDataTags == None:
#        transferAccsAllFGs = np.zeros(numFGs)
#    else:
